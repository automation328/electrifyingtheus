// Shared analytics aggregation over the site_analytics table. Used by BOTH:
//   • api/analytics.ts — the password-gated internal dashboard (/admin)
//   • api/admin.ts      — op:"analytics", editor-gated, embedded in the CMS
//                         (/admin/content/statistics)
// Keeping one implementation avoids the two surfaces drifting apart. The leading
// underscore keeps Vercel from treating this file as its own endpoint.
...
/**
 * A single visitor's page-by-page journey over the last 90 days, grouped by
 * session (newest session first).
 */
export async function visitorJourney(db: SupabaseClient, visitorId: string): Promise<Result<unknown>> {
  const vid = visitorId.slice(0, 80);
...
/** Aggregated dashboard summary for a time range (totals, series, pages, visitors, breakdowns). */
export async function analyticsSummary(db: SupabaseClient, rangeInput: unknown): Promise<Result<unknown>> {
  const rangeKey = normalizeRange(rangeInput);
  ...
  const pages = [...pageViews.entries()]
    .map(([key, v]) => ({ key, count: v, visitors: pageVisitors.get(key)?.size || 0 }))
