...
// GM nameplate-matched pairs — each picks a GM gas model and its GM electric
// counterpart. Drives the left-rail class picker in the "Compare by GM class"
// section. Three of the five are exact nameplate matches (Equinox, Silverado,
// Escalade), so the switch story is as direct as it gets.
type CompareClass =
  | "compact-suv" | "mid-suv" | "small-suv" | "pickup" | "full-suv";

const CLASS_OPTIONS: {
  key: CompareClass; label: string; iconSrc: string; gas: string; ev: string;
}[] = [
  { key: "compact-suv", label: "Compact SUV",   iconSrc: iconCompactSedan, gas: "chevy-trailblazer", ev: "chevy-bolt-ev" },
  { key: "small-suv",   label: "Small SUV",     iconSrc: iconSmallSuv,     gas: "chevy-equinox",     ev: "chevy-equinox-ev" },
  { key: "mid-suv",     label: "Mid-Size SUV",  iconSrc: iconMidsizeSedan, gas: "cadillac-xt5",      ev: "cadillac-lyriq" },
  { key: "full-suv",    label: "Full-Size SUV", iconSrc: iconFullSuv,      gas: "cadillac-escalade", ev: "cadillac-escalade-iq" },
  { key: "pickup",      label: "Pickup",        iconSrc: iconPickup,       gas: "chevy-silverado",   ev: "chevy-silverado-ev" },
];
...
// This calculator pits any current gas car against a *GM* EV. The gas side is
// the full site catalog; the EV side is the curated GM lineup (src/data/gm-evs),
// sourced from the GM EV knowledge base and used only on this page.
const gasVehicles = getVehiclesByType("gas");
const evVehicles = GM_EVS;
// The page's vehicle universe: every gas car (any brand) + the GM EV lineup.
// Used for all id lookups and handed to the EV-match engine, so every
// recommended substitute is a GM electric vehicle.
const gmVehicles = [...gasVehicles, ...GM_EVS];
...
const DEFAULT_STATE: CalcState = {
  gasId: "chevy-equinox", evId: null, stateCode: "CA", homeCharging: true,
...
const GmEvVsGas = () => {
  ...
  const [gasMake, setGasMake] = useState(() => { const v = gmVehicles.find((x) => x.id === searchParams.get("car")); return v ? makeOf(v.name) : ""; });
  const [evMake, setEvMake] = useState(() => { const v = gmVehicles.find((x) => x.id === searchParams.get("ev")); return v ? makeOf(v.name) : ""; });
  ...
  useEffect(() => { const v = gmVehicles.find((x) => x.id === gasId); if (v) setGasMake(makeOf(v.name)); }, [gasId]);
  useEffect(() => { const v = gmVehicles.find((x) => x.id === evId); if (v) setEvMake(makeOf(v.name)); }, [evId]);
  ...
  const FALLBACK_GAS = gmVehicles.find((v) => v.id === "chevy-equinox") ?? gmVehicles.find((v) => v.type === "gas")!;
  const FALLBACK_EV = gmVehicles.find((v) => v.id === "chevy-equinox-ev") ?? gmVehicles.find((v) => v.type === "ev")!;
  ...
  const pickMake = (mk: string, id: string, setMake: (s: string) => void, setId: (s: string) => void) => {
    setMake(mk);
    const cur = gmVehicles.find((v) => v.id === id);
  ...
  const matches = useMemo(() => (gasSel ? recommendEvs(gas, gmVehicles) : []), [gasSel, gas]);
  ...
  const ogShareUrl = () => {
    if (typeof window === "undefined") return "/gm-ev-vs-gas";
  ...
  const nativeShare = async () => {
    const { url, text } = buildShare();
    try {
      await navigator.share({ title: "GM EV vs Gas Calculator", text, url });
    } catch {
        ...
        <div className="container px-4 max-w-5xl">
          <div className="text-center">
            ...
            <p className="evg-rise text-muted-foreground text-lg md:text-xl max-w-2xl mx-auto" style={{ animationDelay: "0.24s" }}>
              See what switching your current gas car for a GM EV — the Equinox EV, Silverado EV,
              Cadillac LYRIQ and more — really costs to run. How lopsided the fight is depends
              entirely on where you live. Pick your state to load it into the ring.
            </p>
      ...
      <main className="flex-1 relative z-10">
        ...
        <section className="py-16 md:py-20">
          <div className="container px-4 max-w-5xl">
            ...
            <p className="text-muted-foreground mb-8 max-w-2xl">
              ...
              the right GM EVs and preset everything else from {rates.name} averages.
              ...
              <div className="p-6 md:p-8">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground">Pick the GM EV you're eyeing vs the car you drive today</div>
                  </div>
                </div>
                ...
                <div className="grid md:grid-cols-[1fr_auto_1fr] md:items-end gap-5 md:gap-4">
                  <div>
                    <label className="text-[11px] font-semibold uppercase tracking-[0.14em] mb-2 flex items-center gap-1.5" style={{ color: EV_COLOR }}>
                      <Zap className="w-3.5 h-3.5" /> GM electric car
                    </label>
            ...
            <div className="mb-6 flex flex-col items-center gap-2">
              ...
              {!bothSelected && (
                <p className="text-xs text-muted-foreground">
                  Select your current gas car and a GM electric car to calculate.
                </p>
              )}
            </div>
            ...
            <div className="mb-6">
              <div className="flex items-center justify-between gap-3 mb-3">
                <h3 className="font-charge text-xl text-foreground flex items-center gap-2">
                  <Sparkles className="w-5 h-5 text-primary" /> Your best GM EV matches
                </h3>
                <span className="text-xs text-muted-foreground">GM EVs class-matched to the {gas.name} — tap to load one above</span>
              </div>
              ...
              <div className="relative overflow-hidden mb-8 rounded-3xl gradient-hero text-primary-foreground p-7 md:p-9 shadow-elevated text-center">
                ...
                <div className="relative">
                  ...
                  <p className="text-primary-foreground/90 max-w-xl mx-auto mb-6">
                    Add the live GM&nbsp;EV&nbsp;vs&nbsp;Gas Calculator and EVan, your E-Mobility Advisor, to
                  ...
                  <Link to="/contact-us">
                    <Button className="rounded-xl bg-white text-primary font-semibold hover:bg-white/90">
                      <Sparkles className="w-4 h-4" /> Add this GM EV vs Gas Calculator to my site
                    </Button>
                  </Link>
                </div>
              </div>
          ...
          <div className="container px-4 max-w-5xl">
            ...
            <div className="mb-8">
              ...
              <p className="text-muted-foreground mt-2">
                Pick a class to see a GM gas model and its GM electric counterpart, head to head.
              </p>
            </div>
        ...
        {embed && (
          <div className="container px-4 max-w-5xl pb-8 text-center">
            <a href="/gm-ev-vs-gas" target="_blank" rel="noopener noreferrer"
